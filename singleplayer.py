...
# Returns resource frequency for a certain biome type
resource_frequency = [[0.15,0.15,0.15,0.15], [0.15,0.1,0.15,0.15], [0.05, 0.05, 0.15, 0.05], [0,0.15,0,0]]
resource_frequency = numpy.array(resource_frequency, dtype = float)

# Generate biomes
num_biomes = 20 # int(input("Number of Biomes: "))
# For deserts, add oases which are water resource and add food/wood to adjacent tiles
biomes = []
for i in range(num_biomes):
    biomes.append((random.randint(0,map_width-1), random.randint(0,map_height-1), int((random.randint(2,8))/2)))
    # 0 is nothing, 1 in plains, 2 tundra, 3 is desert, 4 is ocean
...
class Tile(pygame.sprite.Sprite):
    def __init__(self,type):
        ...
        for i in range(num_resource_types):
            if random.random() < resource_frequency[type-1][i]/2:
                resources.append(i)
                pygame.draw.rect(self.image,resource_colors[i],pygame.Rect(0,0,40,40))
        resources = numpy.array(resources, dtype = int)
...
for x in range(map_width):
    ...
    for y in range(map_height):
        ...
        map_tiles[x].append(Tile(biome_type))
            
map_tiles = numpy.array(map_tiles, dtype = object)
